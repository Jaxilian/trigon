...
static inline void md5_upd(md5_ctx_t* ctx, uint8_t* input_buffer, size_t input_len) {
    uint32_t input[16];
...
void md5_str(char* input, md5_t result) {
    md5_ctx_t ctx;
    md5_ini(&ctx);
    md5_upd(&ctx, (uint8_t*)input, strlen(input));
    md5_build(&ctx);
...
void md5_file(FILE* file, md5_t result) {
    ...
    while ((input_size = fread(input_buffer, 1, 1024, file)) > 0) {
        md5_upd(&ctx, (uint8_t*)input_buffer, input_size);
    }
