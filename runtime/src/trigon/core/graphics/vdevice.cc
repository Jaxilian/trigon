...
void vdevice_t::create_instance() {
...
void vdevice_t::get_queue_families(
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(physical_device, &queue_fam_count, queue_families.data());
...
bool vdevice_t::device_suitable(VkPhysicalDevice physical_device, VkSurfaceKHR surface) {
    ...
    queue_family.graphics_family = graphics_family;
    queue_family.present_family = present_family;
    return valid && ext_supported && swap_ready &&
        supported_features.samplerAnisotropy;
}

void vdevice_t::get_physical_device(VkSurfaceKHR surface) {
...
void vdevice_t::bind_device(VkSurfaceKHR surface) {
    if (device != nullptr) return;
    get_physical_device(surface);

    std::vector<VkDeviceQueueCreateInfo> queue_infos;
    std::set<uint32_t> unique_families = { queue_family.graphics_family, queue_family.present_family };
    ...
    VkQueue pque;
    VkQueue gque;
    vkGetDeviceQueue(dev, queue_family.graphics_family, 0, &gque);
    vkGetDeviceQueue(dev, queue_family.present_family, 0, &pque);
    graphics_queue = gque;
    present_queue = pque;

    VkCommandPoolCreateInfo pool_info = {};
    pool_info.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
    pool_info.queueFamilyIndex = queue_family.graphics_family;
    pool_info.flags =
...
VkFormat vdevice_t::find_format(
