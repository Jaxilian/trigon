...
void gfx_default_pipeline_info(gfx_pipeline_info_t* config_info) {
    ...
    config_info->dynamic_states = dynamic_states;
    config_info->dynamic_state_count = sizeof(dynamic_states) / sizeof(dynamic_states[0]);

    config_info->dynamic_state_info = (VkPipelineDynamicStateCreateInfo){
        .sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO,
        .dynamicStateCount = config_info->dynamic_state_count,
        .pDynamicStates = config_info->dynamic_states,
    };

    // Initialize pointers to NULL
    config_info->binding_descriptions = NULL;
    config_info->binding_description_count = 0;
    config_info->attribute_descriptions = NULL;
    config_info->attribute_description_count = 0;
