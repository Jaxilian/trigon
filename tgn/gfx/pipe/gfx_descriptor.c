...
void gfx_desc_pool_setup(gfx_pipe_t* pipe,
    ...
    uint32_t set_count)
{
    // If we already have a pool, skip
...
void gfx_desc_pool_alloc(gfx_pipe_t* pipe,
    shader_res_set_t sets[MAX_DESC_SETS_IN_USE],
    uint32_t set_count,
    VkDescriptorSet* out_sets)
{
    VkDevice dev = gfx_dev()->device;
    if (!dev || pipe->descriptor_pool == VK_NULL_HANDLE || !out_sets) {
        debug_err("missing arguments for descriptor allocation\n");
        return;
    }

    VkDescriptorSetLayout layouts[MAX_DESC_SETS_IN_USE];
    for (uint32_t i = 0; i < set_count; ++i) {
    ...
    if (vkAllocateDescriptorSets(dev, &alloc_info, out_sets) != VK_SUCCESS) {
