...
bool input_get_key(enum KEYCODE code) {
	return glfwGetKey(ctx->win.handle, code);
}
...
void input_sync() {
	...
	double x, y;
	glfwGetCursorPos(ctx->win.handle, &x, &y);
	cursor[0] = (float)x;
	cursor[1] = (float)y;
	...
	if (!mouse_initialized) {
		cursor_prev[0] = cursor[0];
		cursor_prev[1] = cursor[1];
		glfwSetKeyCallback(ctx->win.handle, window_input_cb);
		glfwSetCursorPosCallback(ctx->win.handle, window_mouse_callback);
		glfwSetMouseButtonCallback(ctx->win.handle, window_mouse_button_callback);
...
void input_cursor_lock(bool value) {
	...
	mouse_locked = value;
	if (mouse_locked) {
		glfwSetInputMode(ctx->win.handle, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
	}
	else {
		glfwSetInputMode(ctx->win.handle, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
	}
}

bool input_mouse_clicked(int id) {
	return glfwGetMouseButton(ctx->win.handle, id);
}
